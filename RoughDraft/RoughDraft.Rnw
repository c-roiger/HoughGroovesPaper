...
\usepackage{amsmath,amssymb,mathrsfs,fancyhdr,syntonly,lastpage,hyperref,enumitem,graphicx,subcaption, tikz, caption, float, booktabs}
...
While not explicitly stated in this work, the images in \cite{chu2013} make it clear that all of the bullet images upon which the researchers are running their edge detection methods have the Groove Engraved Areas (GEAs) of the bullet scan removed. While no method for groove identification is proposed in the three previous research publications cited, it is reasonable to speculate that such areas were manually identified. If so, this highlights the need for automated and unbiased identification of groove engraved areas since the identification of the LEA in bullet scans on the scale necessary to create some sort of database is time intensive and also introduces another element of human intervention in the bullet identification process. Part of the problem with the existing research presented in this paper is that the methods are not described in enough detail to allow for replication of results from external researchers. In addition, the manual intervention results implicit in \cite{chu2013} are not available for public scrutiny or use. 


To evaluate the accuracy of our groove identification we will use a metric devised by Kiegan Rice, Nate Garton, and Heike Hofmann \cite{rice-2019}. The idea behind this metric is that a robust LOESS is fit to model the curvature of the bullet scan. This LOESS model predicts height values, $\hat{z_i}$as a function of location $x_i$ on the bullet land by estimating values $\beta_0, \beta_1$ that minimizes the following equation:
\begin{center}
  $\text{arg} \ \underset{\beta}{\min} \overset{n}{\underset{k=1}{\sum}} w_k(x_i) (z_k-(\beta_0 + \beta_1 x_k))^2$
\end{center}

where $w_k(x_i)$ is a weight associated with each data point $x_k$ based on its distance from $x_i$. The researchers then create a robustness weight, $\delta_k$ which is based on the magnitude of each residual, $e_k = z_k - \hat{z_k}$. The process of fitting the robust LOESS model is as follows: 
\begin{enumerate}
  ...
  \item Obtain predicted height values $\hat{z_k}$ and corresponding residual values $e_k = z_k - \hat{z_k}$.
  \item Calculate updated robustness weights using residual values $e_k$:
  \begin{center}
    $\delta_k \times w_k(x_i) = \left( 1 - \left( \dfrac{e_k}{6 * MAD} \right)^2 \right)^2$ if $\left| \dfrac{e_k}{6*MAD}\right| < 1$ 
  \end{center}
  where MAD is the median absolute deviation of residuals $e_k$.
  \item Repeat steps 1-3 with updated weights at each iteration for $m$ iterations, with 20 set as a default.
  \item After $m = 20$ iterations, fit a LOESS model and obtain residual values $e_i$ for each data point $(x_i, z_i)$
\end{enumerate}

Once these residuals are obtained, the area between the predicted groove location and the manually identified groove location is calculated. More formally, we define the following:
\begin{itemize}
  \item $\mathbf{x_{\gamma}}$: the predicted groove x-location
  \item $\mathbf{x_m}$: the manually identified groove x-location
  \item $\mathbf{e_i}$: The robust LOESS residual value for each data point
  \item $\mathbf{r_s}$: The resolution of the x3p scan
  \item $\mathbf{j_{min}}$: x-location index of $\min(x_{\gamma}, x_m)$ 
  \item $\mathbf{j_{max}}$: x-location index of $\max(x_{\gamma}, x_m)$ 
\end{itemize}

Thus the AOM score can be calculated using 
\begin{center}
  $\overset{j_{max}}{\underset{i = j_{min}}{\sum}}|e_i|*r_s$
\end{center}


<<AOM-motivation, echo = FALSE, fig.align="center", message=FALSE, warning = F, fig.height = 4, fig.cap= "Area of misidentification between well-specified and mis-specified signatures shown in purple.">>=
sigs1 <- cc_get_signature(ccdata, grooves, span1 = 0.75, span2 = 0.05)
grooves.bad <- list("groove" = c(grooves$groove[1] -50, grooves$groove[2]))
...
@
 ...
 This can best be described as the integral between two bullet signatures, one where the signature is based on the manually identified groove location and another signature based on our groove estimate, thus yielding an area in $\text{microns}^2$, as shown in Figure~\ref{fig:AOM-motivation}. This AOM score and can be used as a score to judge how closely our estimated groove location is to the manually identified groove location. 

\subsection{Brief Overview of Hough Transforms}


In broad terms, the Hough Transform is a feature extraction technique for detecting shapes in an image. For any given point in the (x,y)-plane of our image we can calculate a corresponding line in the feature space as shown in Figure~\ref{fig:parametrization}. Points that lie on the same line will have lines that intersect in the feature space. The point of intersection in the feature space, then corresponds to the parameters used to describe the edge in the (x,y)-plane that the two detected points lie upon. A two dimensional array called the ``accumulator array" is used to keep track of features detected.  The accumulator array covers the entirety of the feature space separated into a user-specified number of bins. For each set of features detected the bin in the accumulator array associated with that set of features is incremented. So in theory, strong features should have higher values in their associated bin because there is a larger number of pixels detected that all have the same set of features in the feature space. 
...
We make the assumption that scans are oriented properly and as such, most Hough lines that correlate to the GEAs will be roughly vertical with some deviations in angle based on scanning technique. It is worth noting that the original output of the `` \texttt{hough\char`_lines}" functions have angles ranging from 0 to $2\pi$. So to make selecting lines from our desired range easier we chose to transform any angle greater than $\pi$, to instead be within the interval 0 to $-\pi$ by subtracting $2\pi$ from the original theta angle. Resulting in all functions having angles in the interval $-\pi$ to $\pi$. Therefore we select only the Hough lines that have theta angles from the positive x-axis less than $\frac{\pi}{16}$ and greater than $\frac{-\pi}{16}$. The parametrization produced by `` \texttt{hough\char`_lines}" is of the form:
...
\end{itemize}
We note that when $\theta$ is equal to 0, the $x_t$ is given by $\rho$ however when $\theta$ is not 0 we utilize the above equation to find that the x-intercept at the top of the bullet land can be given by:
\begin{center}
$x_t$ = $\frac{\rho}{\cos(\theta)}$
\end{center}

We utilize this calculation to find where the estimated Hough line intersects the top and bottom of the bullet land. The $x_t$ was calculated previously, however to calculate $x_b$ we utilize some geometric properties. We can draw a perpendicular line from the $x_t$ to the bottom of the land and another line connecting $x_t$ to $x_b$ creating a right triangle as shown below in Figure \ref{fig:xbottom-graphic-inclusion}. Since both the angle between the orthogonal bisector labeled $\rho$ and the new triangle created from the $x_t$ to the bottom of the land are both right triangles, we know that the angle at the top of the newly formed triangle is equivalent to $\theta$. Since the $x_t$ is known, we can use an elementary geometry technique to calculate the distance labeled $\delta$ in the diagram below. Since $\tan(\theta)$ is equivalent to the proportion of the length of the ``opposite" side over the ``adjacent" side of the right triangle, we know that $\delta = \tan(\theta)*\text{h}$. So then $x_b = x_t - \delta$. 
...
Once features are chosen to describe both the left and right hand grooves of the bullet land the \textbf{get\_grooves\_hough} function translates these features into two equations for lines, where the left-hand and right-hand groove are denoted $\gamma_{lh}, \gamma_{rh}$, respectively. It is worthwhile to note at this point that our calculations have been in terms of pixels detected in an image rather than the microns of the bullet scan. So we must transform our equations of lines from being in terms of pixels to microns. This is accomplished inside the \textbf{get\_grooves\_hough} function by the helper function \textbf{pix\_to\_micron} which subtracts one from the pixel location (since pixels start indexed at one and microns start indexed at zero), then multiplies that pixel location by the resolution of the x3p-scan($r_s$). In addition, we can also introduce manually identified adjusts into our groove equations. Each test set tends to have an optimal adjust value that provides a more accurate groove identification by shifting the automatically identified grooves into the interior of the bullet land by the number of microns specified by the adjust on both the left and right-hand sides of the bullet land. Because we are interested in the slope with respect to the y-direction we calculate our linear equations as follows, where $y_i$ is any y-location on the bullet land and $\alpha$ is the groove adjust:
...
\end{center}
 ...
 Unlike other methods available in the \textit{GrooveFinder} package, the \textbf{get\_grooves\_hough} function does not provide a groove estimate at an optimized crosscut. The output of the  \textbf{get\_grooves\_hough} function are a set of two functions that correspond to the left and right-hand grooves. So to produce an actual estimate of our groove location we need to input a y-location into our two functions. For a single point-estimate we can input the y-location of a single crosscut using the \textbf{x3p\_optimize\_crosscut} function from the \textit{bulletxtrctr} package. However to visualize our groove estimates across the entire bullet land we have created a helper function called \textbf{get\_mask\_hough} that creates a colored mask over our bullet land of our groove estimates as shown in Figure \ref{fig:get-mask-hough-example}. 
...
Below we present the distribution of area of misidentification(AOM) scores for Hough and Rollapply estimates for the Phoenix PD set in Figure \ref{fig:phoenix-score-dist-density}. For the left groove we can see that the Hough method has a much larger accumulation of well-specified grooves than that of the Rollapply since most Hough scores are clustered at lower values. For the right-hand groove, both methods appear to have a large number of scores clustered at lower values. However, for both left and right groove locations the Hough method has a lower spread of AOM scores than that of the Rollapply method.


This conclusion is further emphasize in Figure \ref{fig:phoenix-score-dist-bp} where we have log transformed AOM scores shown as boxplots. Smaller logged scores indicate a more accurate identification of groove locations. For both the right and left-hand grooves for this set we see that the Hough method has a smaller spread in logged AOM scores than the Rollapply method. For the left hand groove, the Hough method appears to have a lower median logged AOM score than the Rollapply method however for the right hand side, the median logged AOM scores is slightly higher. 

<<phoenix-score-dist-density, echo = FALSE, fig.align="center", message=FALSE, warning = F, fig.height=4, fig.cap = "Density Plots of AOM scores for the Phoenix PD set separated by groove side and method">>=
...
<<phoenix-score-dist-bp, echo = FALSE, fig.align="center", message=FALSE, warning = F, fig.height=4, fig.cap = "Boxplots of AOM scores for the Phoenix PD set separated by groove side and method">>=
...
<<phoenix-score-table, echo = FALSE, eval = F, message=FALSE>>=

# Create dataframe with summary statistics
table.df <- phoenix.df %>%
  dplyr::mutate(score = score + 1) %>%
  dplyr::group_by(method, groove_chirality) %>%
  dplyr::summarise(Mean = mean(log(score), na.rm = T, ),
            Variance = var(log(score), na.rm = T),
            Median = median(log(score), na.rm = T),
            Minimum = min(log(score), na.rm = T),
            Maximum = max(log(score), na.rm = T))
table.df <- table.df %>%
  dplyr::rename(`Method` = method, 
                `Groove Chirality`= groove_chirality)

# Kable-extra styling will not work... 
# 
# kableExtra::kbl(table.df, caption = "Table of Summary Statistics of Logged AOM scores by method and groove chirality for the Phoenix PD Set", booktabs = T) %>%
...
\textbf{Method} & \textbf{Groove Chirality} & \textbf{Mean} & \textbf{Variance} & \textbf{Median} & \textbf{Minimum} & \textbf{Maximum} \\ \hline
Hough           & Left                      & 5.755         & 5.131             & 5.648           & 0                & 10.328           \\
Rollapply       & Left                      & 6.821         & 6.176             & 6.551           & 0.319            & 10.943           \\
Hough           & Right                     & 5.045         & 3.241             & 5.515           & 0                & 8.691            \\
Rollapply       & Right                     & 5.482         & 2.526             & 5.165           & 1.472            & 9.658            \\ \hline
\end{tabular}
\caption{Table of summary statistics for logged AOM scores for the Phoenix PD Set}
\label{table:phoenix-table}
\end{table}

In the above table, we include some useful summary statistics of the logged AOM scores. In general the means and variances of both methods for each groove are consistent with the conclusions presented earlier in this section. It appears that for the left groove, the Hough method has a lower average logged AOM score and a lower variance. But for the right groove the Hough estimate has a lower mean, a higher median, and a higher variance. An item of note is that we added 1 to each AOM score before being logged so that AOM scores of 0 can be included in these summary statistics and box plot distributions for both the Phoenix set and the Hamby Set 44.

We see similar results in the Hamby Set 44 for both the left-hand and right-hand groove estimates. In Figure~\ref{fig:hamby44-score-dist-density} we see the density distribution of AOM scores for the Hamby Set 44. For the left hand side, there appears to be a large spread of AOM scores for both the Rollapply and Hough method, however the Hough method has a slightly higher concentration of AOM scores at lower values than the Rollapply method. A similar story can be seen for the right hand groove where both method appear to have a large number of scores clustered near zero, but the Hough method has a larger density of scores at lower values.  

<<hamby44-score-dist-density, echo = FALSE, fig.align="center", message=FALSE, warning = F, fig.height=4, fig.cap = "Density Plots of AOM scores for the Hamby Set 44 separated by groove side and method">>=
...
When the logged AOM scores are displayed as box plots we can easily see that for the left groove the Hough method 
has a smaller spread of logged AOM scores and also has a lower median logged AOM score than that of the Rollapply method. For the right groove it appears that both methods have a similar median, however the distribution of the Hough method is somewhat lower than the Rollapply method. 


<<hamby44-score-dist-bp, echo = FALSE, fig.align="center", message=FALSE, warning = F, fig.height=4, fig.cap = "Distribution of left-hand areas of misidentification for Hamby 44 Set">>=
...
In Table 2, we once again provide summary statistics of the logged AOM scores. For the left-hand grooves, the Hough method has a lower average and standard deviation than that of the Rollapply method. But for the right-hand groove estimate the Hough method may have alower median and mean, but it has a higher variance than that of the Rollapply method.

\begin{table}[ht!]
\begin{tabular}{lllllll}
\textbf{Method} & \textbf{Groove Chirality} & \textbf{Mean} & \textbf{Variance} & \textbf{Median} & \textbf{Minimum} & \textbf{Maximum} \\ \hline
Hough           & Left                      & 6.331         & 3.188             & 6.749           & 0                & 9.291           \\
Rollapply       & Left                      & 6.523         & 3.941             & 7.268           & 0.198            & 9.001           \\
Hough           & Right                     & 4.702         & 5.516             & 4.874           & 0                & 10.267            \\
Rollapply       & Right                     & 5.425         & 3.926             & 5.175           & 0.773            & 9.734            \\ \hline
\end{tabular}
\caption{Table of summary statistics for logged AOM scores for the Hamby Set 44}
\label{table:hamby-table}
\end{table}

<<hamby-score-table, echo = FALSE, eval = F, message=FALSE>>=
...
Overall, the Hough method appears to offer a slight improvement in groove location identification than the Rollapply method presented by \cite{hare2017}. Interestingly enough, the effectiveness of the Hough method varies with the side of the groove being identified. As demonstrated in both the Phoenix PD Set and the Hamby Set 44, it seems as if the identifications for both the Hough and Rollapply method are more accurate on the right hand side of the bullet land. It is worth noting however, that the method we use to evaluate the accuracy of groove identifications (AOM scores) contains a slight bias towards the Rollapply method. The reason for this is that the manual groove identifications were made as adjustments to existing Rollapply groove estimates. So reasonably accurate Rollapply estimates of groove locations were accepted without adjustment as a proper identification while less accurate Rollapply estimates were adjusted accordingly. Regardless, the appeal of the Hough method lies in its relative simplicity as an application of a simple computer vision algorithm and its ability to draw information from the entirety of a bullet scan. Further examination of the precision of some of the heuristics used to define this process would be good candidates for further examination. 
