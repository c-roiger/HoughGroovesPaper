...
Hough           & Left                      & 5.823         & 4.833             & 5.669           & 0.294            & 10.328           \\
Rollapply       & Left                      & 6.805         & 6.329             & 6.550           & -0.976           & 10.943           \\
Hough           & Right                     & 5.211         & 2.493             & 5.598           & 1.054            & 8.691            \\
Rollapply       & Right                     & 5.469         & 2.580             & 5.159           & 1.211            & 9.657            \\ \hline
\end{tabular}
\caption{Table of summary statistics for logged AOM scores for the Phoenix PD Set}
\label{table:phoenix-table}
\end{table}

In the above table, we include some useful summary statistics of the logged AOM scores. In general the means and variances of both methods for each groove are consistent with the conclusions presented earlier in this section. It appears that for both the left and right groove, the Hough method has a lower average logged AOM score and a lower variance. An item of note is that ten Hough groove estimates were omitted from the above calculations because these estimates had a logged AOM score of negative infinity which corresponds to an AOM score of 0. 
...
In Table 2, we once again provide summary statistics of the logged AOM scores. For this set, thirty seven Hough groove estimates were omitted from the summary statistic calculations because these estimates had a logged AOM score of negative infinity (AOM score of 0). For both left and right-hand grooves, the Hough method has a lower average and standard deviation than that of the Rollapply method. However the distributions are much closer in shape and spread for the right-hand groove estimate for this particular set.

\begin{table}[ht!]
\begin{tabular}{lllllll}
\textbf{Method} & \textbf{Groove Chirality} & \textbf{Mean} & \textbf{Variance} & \textbf{Median} & \textbf{Minimum} & \textbf{Maximum} \\ \hline
Hough           & Left                      & 6.374         & 2.956             & 6.775           & 0.3613           & 9.291           \\
Rollapply       & Left                      & 6.505         & 4.111             & 7.267           & -1.553           & 9.001           \\
Hough           & Right                     & 5.149         & 3.856             & 5.144           & -2.561           & 10.267            \\
Rollapply       & Right                     & 5.401         & 4.077             & 5.169           & 0.153            & 9.734            \\ \hline
\end{tabular}
