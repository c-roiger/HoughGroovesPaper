...
*** Have a choice here, can compare hough transform results to those of the LASSO and BCP methods and discuss the ways in which the Hough transform fails. Or can discuss the hough transform on its own. I for now will start with comparing the Hough transform to the other methods.***

In order to assess the effectiveness of the Hough Transform method we will use the "Area of Misidentifiaction" score, as described above, to see how far away our prediction is from the manually identified groove location. We will also compare our results to two other methods of groove identification to see how well this new method compares to tools previously developed by CSAFE researchers. In total there will be three methods for comparison:

\begin{enumerate}
\item Hough Transform
\item Logistic LASSO
\item Bayesian changepoint
\end{enumerate}
